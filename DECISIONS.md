...
- [2026-08-16] **Each of the three certs Ming is sitting gets to one hour, rather than building the full roadmap** — decided-by: user · confidence: 0.9 · why: the ask was an hour of study audio per exam, and the roadmap's nineteen remaining episodes would have been several sessions of authoring for coverage he has not asked for yet; the ten chosen episodes are the highest-weight uncovered domains, so SCS now covers eighty six percent of its paper and SAA has an episode for all four domains · revert: nothing to revert, the remaining roadmap episodes in `docs/aws-cert-track.md` are unchanged and still buildable
- [2026-08-16] **`build-episode` takes a list of blueprints instead of one run per episode** — decided-by: agent · confidence: 0.9 · why: dispatching ten runs left one running, one pending and eight cancelled — `cancel-in-progress: false` protects the executing run but a concurrency group holds only one pending run, so each dispatch cancelled its predecessor; batching also keeps the manifest commit per batch rather than per episode · revert: restore the single-path `echo` in the resolve step, and dispatch one at a time waiting for each to finish
- [2026-08-16] **Batches are capped at three episodes per run** — decided-by: agent · confidence: 0.75 · why: measured 14m54s for a single 11:50 episode, of which about twelve minutes is text-to-speech at roughly real time, so four is the most that fits the sixty-minute job timeout and three leaves room; a run that times out mid-batch leaves audio in R2 with no manifest commit, which is the expensive failure · revert: raise `timeout-minutes` and batch larger
- [2026-08-14] **`deep-dive-2voice` added as a second two-voice template, for practitioner rather than exam content** — decided-by: agent · confidence: 0.85 · why: `cert-prep-2voice` is built around domain weights, distractor patterns and a recall drill, none of which exist outside an exam; the AI infrastructure show needed a `numbers` section and a `failure` section instead, which is a genuinely different format rather than a variant · revert: delete `content/templates/deep-dive-2voice.json` and repoint the show
- [2026-08-14] **The MLOps show targets MLA-C01 despite the version being retired on 28 Sep 2026** — decided-by: agent · confidence: 0.8 · why: MLA-C02's exam guide does not exist yet (the docs URL 404s) and its beta only opens 1 Sep, so C01 is the only version that can be studied from published material today; the deadline is made the cold open of episode one rather than hidden · revert: rewrite against the C02 guide once AWS publishes it
